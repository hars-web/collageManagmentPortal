...
import { markAllRead, markRead, setNotifications } from '../../store/slices/notificationSlice';
...
export default function FacultyNotifications() {
  ...
  return (
    <div>
      <PageHeader
        title="Notifications"
        subtitle={`${unread} unread · faculty notices and updates`}
        crumbs={[{ label: 'Faculty' }, { label: 'Notifications' }]}
        actions={
      ...
      <Card className="divide-y divide-dark-100 dark:divide-dark-800">
        ...
        {list.length === 0 && <EmptyState icon={<BellOff className="h-6 w-6" />} title="All caught up" message="No new notifications" />}
      </Card>
    </div>
  );
}
